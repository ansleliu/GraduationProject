...
{ 
	...
	DAC_InitType.DAC_Trigger = DAC_Trigger_T4_TRGO;	//使用TIM4作为触发源
...
//    DAC_Init(DAC_Channel_2, &DAC_InitType);
	DAC_SetChannel1Data(DAC_Align_12b_R, 1023);  //12位右对齐数据格式设置DAC值
//	DAC_SetChannel2Data(DAC_Align_12b_R, 1023);  //12位右对齐数据格式设置DAC值
...
static void DAC_TIM_Config(void)
{
  ...
  TIM_TimeBaseInitTypeDef    TIM_TimeBaseStructure;
	...
	/* 使能TIM4时钟，TIM4CLK 为72M */
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	
  /* TIM4基本定时器配置 */
 // TIM_TimeBaseStructInit(&TIM_TimeBaseStructure); 
  TIM_TimeBaseStructure.TIM_Period = 0x85;//定时周期 20  
  TIM_TimeBaseStructure.TIM_Prescaler = 0x69; //预分频，不分频
